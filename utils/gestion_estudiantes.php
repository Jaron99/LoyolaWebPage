...
<div class="tab-pane fade show active" id="vista-estudiantes">
    ...
    <div class="card border-0 shadow-sm bg-white">
        <div class="card-header bg-white border-bottom p-3">
            <form action="admin.view.php" method="GET" class="d-flex gap-2 w-100">
                ...
                <div class="input-group shadow-sm">
                    ...
                    <select name="filtro_seccion" class="form-select bg-light border-start-0 fw-semibold text-muted" style="max-width: 250px;">
                        <option value="">Todos los Grados</option>
                        <?php foreach ($listaSecciones as $sec): ?>
                            <option value="<?php echo $sec['id_seccion']; ?>" <?php echo (isset($_GET['filtro_seccion']) && $_GET['filtro_seccion'] == $sec['id_seccion']) ? 'selected' : ''; ?>>
                                <?php echo htmlspecialchars($sec['nombre_completo']); ?>
                            </option>
                        <?php endforeach; ?>
                    </select>

                    <span class="input-group-text bg-white border-start-0 border-end-0 text-muted px-2 d-none d-md-flex">|</span>

                    <span class="input-group-text bg-white border-start-0 border-end-0"><i class="bi bi-search text-muted"></i></span>
                    <input type="text" name="buscar_alumno" class="form-control border-start-0 bg-white" placeholder="Buscar por nombre o apellido..." value="<?php echo isset($_GET['buscar_alumno']) ? htmlspecialchars($_GET['buscar_alumno']) : ''; ?>">

                    <button type="submit" class="btn btn-primary fw-bold px-4">Filtrar</button>
                    <?php if (!empty($_GET['buscar_alumno']) || !empty($_GET['filtro_seccion'])): ?>
                        <a href="admin.view.php?tab=estudiantes" class="btn btn-outline-secondary">Limpiar</a>
                    <?php endif; ?>
                </div>
            </form>
        </div>
